...
def check_data_center_feasibility(data_center: DataCenter, load_profile: Union[np.ndarray, list],
                                  it_load_profile: np.ndarray, n_ts: int, delta_t_s: int) -> float:
    """
    Check heat pump load profile obtained from the DC module

    :param data_center: the DataCenter object for which load profile must be checked
    ...
    prop_nonzero_it_load = data_center.COP_CS / (data_center.EER * (data_center.COP_HP - 1) * delta_t_s) * it_load_profile[nonzero_it_load_ts]
    ...
    return calculate_infeas_score(n_infeas_check=n_infeas_check, infeas_list=infeas_list,
...
def check_industrial_cons_feasibility(load_profile: np.ndarray, batt_capa: float, batt_max_power: float,
                                      charge_eff: float, discharge_eff: float, n_ts: int,
                                      delta_t_s: int) -> float:
    """
